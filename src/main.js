...
function clubJerseyMaterial(skin,view='front'){
  const c=document.createElement('canvas');c.width=c.height=32;const q=c.getContext('2d');q.imageSmoothingEnabled=false;
  const p=colorCss(skin.color),a=colorCss(skin.accent);q.fillStyle=p;q.fillRect(0,0,32,32);
  if(skin.clubStyle==='arsenal'){q.fillStyle='#ffffff';q.fillRect(0,0,7,32);q.fillRect(25,0,7,32);q.fillStyle='#172b50';q.fillRect(11,0,10,3)}
  else if(skin.clubStyle==='bayern'){q.fillStyle='#f2f2e9';q.fillRect(0,4,32,4);q.fillStyle='#8d1020';q.fillRect(0,17,32,3);q.fillStyle='#ffffff';q.fillRect(12,0,8,3)}
  else if(skin.clubStyle==='real-gk'){q.fillStyle='#111d2c';q.fillRect(0,0,5,32);q.fillRect(27,0,5,32);q.fillStyle=a;q.fillRect(12,0,8,3);q.fillStyle='#ffffff';q.fillRect(4,5,4,3)}
  else if(skin.clubStyle==='mancity'){q.fillStyle=a;q.fillRect(0,0,5,32);q.fillRect(27,0,5,32);q.fillStyle='#ffffff';q.fillRect(11,0,10,3)}
  else if(skin.clubStyle==='barca'){q.fillStyle=a;q.fillRect(5,0,5,32);q.fillRect(15,0,5,32);q.fillRect(25,0,5,32);q.fillStyle='#e7c14b';q.fillRect(12,0,2,32)}
  else if(skin.clubStyle==='real'){q.fillStyle=a;q.fillRect(0,0,4,32);q.fillRect(28,0,4,32);q.fillStyle='#d9b34d';q.fillRect(12,0,8,3)}
  if(view!=='side'){q.font=`900 ${view==='back'?18:13}px monospace`;q.textAlign='center';q.textBaseline='middle';q.lineWidth=3;q.strokeStyle='#101820';q.fillStyle='#ffffff';q.strokeText(String(skin.number),16,view==='back'?18:20);q.fillText(String(skin.number),16,view==='back'?18:20)}
  const texture=new THREE.CanvasTexture(c);texture.magFilter=THREE.NearestFilter;texture.minFilter=THREE.NearestMipmapLinearFilter;texture.colorSpace=THREE.SRGBColorSpace;return new THREE.MeshStandardMaterial({map:texture,roughness:.9});
}
function clubKit(skin){let kit=clubKits.get(skin.id);if(kit)return kit;const side=clubJerseyMaterial(skin,'side'),front=clubJerseyMaterial(skin,'front'),back=clubJerseyMaterial(skin,'back');const shortsColor=skin.clubStyle==='arsenal'?0xf2f2ee:skin.clubStyle==='real-gk'?0x17202c:skin.clubStyle==='barca'?0x172b68:skin.clubStyle==='bayern'?0xd71920:skin.clubStyle==='real'?0xf2f2ee:0xffffff;kit={body:[side,side,side,new THREE.MeshStandardMaterial({color:shortsColor,roughness:.92}),front,back],shorts:new THREE.MeshStandardMaterial({color:shortsColor,roughness:.92}),side};clubKits.set(skin.id,kit);return kit}
...
const clubPackPlayers=CLUB_PACK_SKINS.map(skin=>{
  if(skin.sourceIndex!==undefined)return {...RESCUE_PLAYERS[skin.sourceIndex],name:`${skin.shortName}·俱乐部`};
  return {name:'裤袜·俱乐部',country:'比利时',team:'belgium',number:skin.number,appearance:skin.appearance};
});
const clubStarHeads=clubPackPlayers.map(player=>{const starHead=createStarHead(player);starHead.position.y=2.27;starHead.scale.setScalar(1.1);starHead.visible=false;hero.add(starHead);return starHead});
...
function loadSkinProfile(){try{const saved=JSON.parse(localStorage.getItem(SKIN_PROFILE_KEY)||'{}');return{claimedMedong:Boolean(saved.claimedMedong),rescueCompleted:Boolean(saved.rescueCompleted),rosisterCompleted:Boolean(saved.rosisterCompleted),clubPackSkins:Array.isArray(saved.clubPackSkins)?saved.clubPackSkins:[],packCount:Math.max(0,Number(saved.packCount)||0),selected:typeof saved.selected==='string'?saved.selected:'prisoner'}}catch{return{claimedMedong:false,rescueCompleted:false,rosisterCompleted:false,clubPackSkins:[],packCount:0,selected:'prisoner'}}}
...
function applyHeroSkin(){const available=availableSkinIds();if(!available.has(skinProfile.selected))skinProfile.selected='prisoner';const skin=SKIN_CATALOG.find(item=>item.id===skinProfile.selected)||SKIN_CATALOG[0];for(const starHead of heroStarHeads)starHead.visible=false;for(const starHead of clubStarHeads)starHead.visible=false;heroMedongFrontHair.visible=heroMedongBackHair.visible=heroMedongSideHair.visible=false;head.visible=hair.visible=true;ponytail.visible=skin.id==='ponytail';arm1.material=arm2.material=M.skin;heroSleeve1.material=heroSleeve2.material=M.orange;heroSleeve1.scale.y=heroSleeve2.scale.y=1;heroSleeve1.position.y=heroSleeve2.position.y=.34;shoe1.material=shoe2.material=M.black;
  ...
  else if(skin.kind==='club'){const index=CLUB_PACK_SKINS.findIndex(item=>item.id===skin.id),player=clubPackPlayers[index],kit=clubKit(skin);body.material=kit.body;leg1.material=leg2.material=kit.shorts;head.visible=hair.visible=ponytail.visible=false;clubStarHeads[index].visible=true;arm1.material=arm2.material=starMaterial(player,'skin');heroSleeve1.material=heroSleeve2.material=kit.side}
...
function syncPackUi(){packCountEl.textContent=String(skinProfile.packCount);packCountPanel.textContent=String(skinProfile.packCount);if(packReward.classList.contains('hidden')===false&&!packResult.classList.contains('hidden'))packCopy.textContent=`剩余未开启卡包：${skinProfile.packCount} 包`;openPackWin.classList.toggle('hidden',skinProfile.packCount<=0)}
function openPackOverlay(){packResult.classList.add('hidden');packResult.classList.remove('duplicate');packCopy.textContent=`未开启卡包：${skinProfile.packCount} 包`;packReward.classList.remove('hidden');syncPackUi()}
function openOneClubPack(){if(skinProfile.packCount<=0){announce('当前没有未开启的卡包');return}const skinId=rollClubPack(),skin=CLUB_PACK_SKINS.find(item=>item.id===skinId),duplicate=skinProfile.clubPackSkins.includes(skinId);skinProfile.packCount--;if(!duplicate)skinProfile.clubPackSkins.push(skinId);saveSkinProfile();renderSkinGrid();syncPackUi();packResult.classList.remove('hidden');packResult.classList.toggle('duplicate',duplicate);packResult.innerHTML=duplicate?`抽中了 <b>${skin.name}</b>！<br>这套皮肤已经拥有，本次不重复增加。`:`🎉 抽中了 <b>${skin.name}</b>！<br>已加入皮肤衣柜，可在任意模式使用。`;openPackButton.textContent=skinProfile.packCount?'再开一包':'关闭';applyHeroSkin()}
function completeGame(t=performance.now()){if(state.won)return;state.won=true;state.wonTime=(t-state.start)/1000;beep(880,.4);haptic([50,40,100]);winTitle.textContent=activeConfig().winTitle;winCopy.textContent=activeConfig().winCopy;document.querySelector('#finalTime').textContent=state.mode==='math'?'PASS':format(state.wonTime);saveBest(state.wonTime);skinProfile.packCount++;if(state.mode==='rescue'){skinProfile.rescueCompleted=true;unlockReward.textContent='🏆 已解锁其余球星皮肤！ · 🎁 获得 1 包俱乐部球衣卡包'}else if(state.mode==='rosister'){skinProfile.rosisterCompleted=true;unlockReward.textContent='🥕 萝姐皮肤已解锁！ · 🎁 获得 1 包俱乐部球衣卡包'}else unlockReward.textContent='🎁 获得 1 包俱乐部球衣卡包！';saveSkinProfile();renderSkinGrid();syncPackUi();unlockReward.classList.remove('hidden');mathPanel.classList.add('hidden');rosisterPanel.classList.add('hidden');document.querySelector('#win').classList.remove('hidden')}
...
document.querySelector('#play').onclick=()=>startMode('escape');document.querySelector('#playMath').onclick=()=>startMode('math');document.querySelector('#playRescue').onclick=()=>startMode('rescue');document.querySelector('#reset').onclick=()=>reset(true);document.querySelector('#again').onclick=()=>reset(true);document.querySelector('#changeMode').onclick=returnToModes;document.querySelector('#changeModeWin').onclick=returnToModes;document.querySelector('#packs').onclick=openPackOverlay;openPackWin.onclick=openPackOverlay;openPackButton.onclick=openOneClubPack;document.querySelector('#closePack').onclick=()=>packReward.classList.add('hidden');let sound=true;document.querySelector('#sound').onclick=e=>{sound=!sound;audio.setEnabled(sound);e.currentTarget.textContent=sound?'🔊':'🔇'};document.querySelector('#pauseButton').onclick=()=>setPaused(true);document.querySelector('#resume').onclick=()=>setPaused(false);document.querySelector('#pauseRestart').onclick=()=>{reset(true);audio.resume()};qualitySelect.onchange=e=>{autoDegraded=false;perfState.lowSamples=0;applyQuality(e.target.value);announce(`画质：${e.target.selectedOptions[0].text}`)};
...
skinReward.classList.toggle('hidden',skinProfile.claimedMedong);renderSkinGrid();syncPackUi();applyQuality(qualityMode,false);showBest();reset(true);const shortcut=launchShortcut(location.search);if(shortcut)startShortcut(shortcut);tick(0);
