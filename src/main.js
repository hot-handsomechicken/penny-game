...
function paintFineFringe(q,player){q.fillStyle=colorCss(player.appearance.hair);
  if(player.name==='萝姐'){
    ...
    canvasPolygon(q,[[12,12],[25,14],[29,31],[25,43],[20,36],[18,24]]);canvasPolygon(q,[[39,13],[54,14],[51,34],[45,43],[42,28]]);
...
const HAIR_FRONT_PROFILES={
  ...
  'lms-short-undercut':[{points:[[-.55,.1],[-.55,.48],[-.4,.58],[-.25,.48],[-.08,.62],[.12,.5],[.32,.6],[.55,.48],[.55,.1],[.48,.18],[-.48,.18]]}],
  'kk-long-fringe':[{points:[[-.55,.1],[-.55,.56],[-.4,.62],[-.22,.5],[-.08,.68],[.08,.52],[.25,.72],[.55,.58],[.55,.1],[.48,.18],[-.48,.18]]}],
  'edg-flat-crop':[{points:[[-.55,.1],[-.55,.48],[-.4,.55],[-.3,.66],[-.15,.56],[0,.68],[.16,.56],[.3,.64],[.45,.55],[.55,.48],[.55,.1],[.48,.18],[-.48,.18]]}],
  'bzm-widow':[{points:[[-.55,.1],[-.55,.48],[-.32,.57],[-.12,.42],[0,.3],[.12,.42],[.35,.57],[.55,.48],[.55,.1],[.48,.18],[-.48,.18]]}],
  'bald':[],
...
function reset(full=false){
  ...
  if(full){
    ...
    hunterState.active=false;hunterState.isRosister=false;hunterState.mode='idle';configureHunterForm(false);hunter.visible=false;hunterWarning.visible=false;hunterHud.classList.add('hidden');hunterHud.classList.remove('danger','hit');hunterMainBar.classList.remove('hidden');mechArms.classList.add('hidden');weaponHud.classList.add('hidden');aimReticle.classList.add('hidden');attackButton.classList.add('hidden');attackLabel.textContent='攻击 [F]';moneyGunModel.visible=false;moneyGunPickup.visible=true;for(const shot of moneyBullets)scene.remove(shot);moneyBullets.length=0;lockGates.forEach((_,s)=>lockGate(s,true));document.querySelector('#win').classList.add('hidden');stageEl.textContent=activeNames()[0];bar.style.width=`${100/activeStageCount()}%`;missionEl.textContent=activeMissions()[0];activeCheckpoints().forEach((c,i)=>setCheckpoint(c,i===0));if(state.mode==='math')startMathStage(0);if(state.mode==='rosister')spawnHunter(0)
