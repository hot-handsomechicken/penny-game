...
import {MEDONG_PRAISE,MODE_CONFIG,RESCUE_PLACEMENTS,RESCUE_PLAYERS,SKIN_CATALOG,createMathQuestion,createToiletLayout,isMathAnswerCorrect,isToiletNoCombat,requiredRescueForCheckpoint,rescueHunterHp,unlockedSkinIds} from './modes.js';
import {
  HERO_HEIGHT,
  HERO_RADIUS,
  intersectsBody,
...
const missions=['翻过牢房警戒线','避开横冲直撞的洗衣车','踩着浮箱越过污水渠','躲开旋转的高温管道','穿过交错的安检闸门','跳过灼热炉台','沿维修踏板爬出风管','穿过带刺封锁区','避开屋顶排风扇','突破假撤离点的封锁','进入监狱厕所','靠近四扇相同的门，凭运气选择','沿厕所地洞爬过下水道','沿维修平台穿过竖井','跳过营房警戒线','避开移动的押运车辆','踩着旧箱越过外圈污水','穿过交错的探照光束','沿脚手架登上外墙','击败梅东，抵达直升机'];
...
const rescueStageNames=Array.from({length:MODE_CONFIG.rescue.stageCount},(_,i)=>i<RESCUE_PLAYERS.length?`营救 ${RESCUE_PLAYERS[i].name} · ${String(i+1).padStart(2,'0')}`:`全员突围 · ${String(i+1).padStart(2,'0')}`),rescueMissions=rescueStageNames.map((_,i)=>i<RESCUE_PLAYERS.length?`寻找钥匙并营救 ${RESCUE_PLAYERS[i].name}`:'护送获救球员继续突围');
...
// 普通模式第 11～13 关：厕所入口、随机四选一隔间与下水道。
// 原来的三段地图只保留给拯救模式，两个模式互不影响。
...
for(const object of [...solids,...hazards]){object.getWorldPosition(toiletRegionPosition);if(toiletRegionPosition.z>=300&&toiletRegionPosition.z<390)object.userData.modes=['rescue']}
...
function toiletBox(x,y,z,w,h,d,m=M.white,solid=false,shadow=true,parent=escapeToiletWorld){const object=box(x,y,z,w,h,d,m,solid,shadow,parent);object.userData.modes=['escape'];return object}
...
const toiletState={layout:['tunnel','corpse','corpse','medong'],opened:-1,resolving:false,timers:[]};
...
const heroFace=faceMaterial(),hero=new THREE.Group();scene.add(hero);const body=cube(1.05,1.2,.65,M.orange),head=cube(.82,.82,.78,[M.skin,M.skin,M.hair,M.skin,heroFace,M.skin]),hair=cube(.86,.28,.81,M.hair),ponytail=cube(.36,.82,.38,M.hair),leg1=cube(.42,.72,.46,M.orange),leg2=cube(.42,.72,.46,M.orange),arm1=cube(.3,1.05,.36,M.skin),arm2=cube(.3,1.05,.36,M.skin),shoe1=cube(.44,.2,.58,M.black),shoe2=shoe1.clone();
body.position.y=1.25;head.position.y=2.27;hair.position.y=2.63;ponytail.position.set(0,2.28,-.48);ponytail.visible=false;leg1.position.set(-.29,.45,0);leg2.position.set(.29,.45,0);arm1.position.set(-.72,1.3,0);arm2.position.set(.72,1.3,0);shoe1.position.set(-.29,.12,.08);shoe2.position.set(.29,.12,.08);hero.add(body,head,hair,ponytail,leg1,leg2,arm1,arm2,shoe1,shoe2);
...
for(const [i,player] of RESCUE_PLAYERS.entries()){
  ...
  const follower=new THREE.Group(),jersey=new THREE.MeshStandardMaterial({color:player.color,roughness:.9}),fBody=cube(.92,1,.62,jersey),fHead=cube(.72,.72,.7,[M.skin,M.skin,M.hair,M.skin,faceMaterial(),M.skin]),fLeg1=cube(.34,.65,.4,jersey),fLeg2=cube(.34,.65,.4,jersey),fArm1=cube(.25,.82,.32,M.skin),fArm2=cube(.25,.82,.32,M.skin);
...
const SKIN_PROFILE_KEY='block-break-skin-profile-v1',skinMaterials=new Map();
...
function applyHeroSkin(){const available=availableSkinIds();if(!available.has(skinProfile.selected))skinProfile.selected='prisoner';const skin=SKIN_CATALOG.find(item=>item.id===skinProfile.selected)||SKIN_CATALOG[0];ponytail.visible=skin.id==='ponytail';arm1.material=arm2.material=M.skin;shoe1.material=shoe2.material=M.black;
  ...
  else if(skin.kind==='rescue'){let jersey=skinMaterials.get(skin.id);if(!jersey){jersey=new THREE.MeshStandardMaterial({color:skin.color,roughness:.9});skinMaterials.set(skin.id,jersey)}body.material=leg1.material=leg2.material=jersey;head.material=[M.skin,M.skin,M.hair,M.skin,heroFace,M.skin];hair.material=ponytail.material=M.hair}
...
function syncToiletWorld(){const escape=state.mode==='escape';escapeToiletWorld.visible=escape;for(const object of originalToiletRegion)object.visible=state.mode==='rescue'}
...
function updateRescueHud(){const current=RESCUE_PLAYERS[state.stage],hasKey=rescueState.keys.has(state.stage);rescueHud.querySelector('b').textContent=current&&!rescueState.rescued.has(state.stage)?hasKey?'🔑 已取得本区钥匙':`🔑 寻找 ${current.name} 的钥匙`:'🔑 当前区域已完成';rescueHud.querySelector('span').textContent=`已营救 ${rescueState.rescued.size} / ${RESCUE_PLAYERS.length}`}
function syncRescueObjects(){for(let i=0;i<RESCUE_PLAYERS.length;i++){rescueCages[i].visible=!rescueState.rescued.has(i);rescueKeys[i].visible=!rescueState.keys.has(i)&&!rescueState.rescued.has(i);rescueFollowers[i].visible=state.mode==='rescue'&&rescueState.rescued.has(i)}updateRescueHud()}
...
function reset(full=false){
  if(full||state.mode==='escape')resetToiletDoors(full&&state.mode==='escape');
  if(full){
...
function completeGame(t=performance.now()){if(state.won)return;state.won=true;state.wonTime=(t-state.start)/1000;beep(880,.4);haptic([50,40,100]);winTitle.textContent=activeConfig().winTitle;winCopy.textContent=activeConfig().winCopy;document.querySelector('#finalTime').textContent=state.mode==='math'?'PASS':format(state.wonTime);saveBest(state.wonTime);if(state.mode==='rescue'){skinProfile.rescueCompleted=true;saveSkinProfile();renderSkinGrid()}unlockReward.classList.toggle('hidden',state.mode!=='rescue');mathPanel.classList.add('hidden');document.querySelector('#win').classList.remove('hidden')}
...
function updateRescueMode(dt,t){rescueState.reminderCooldown=Math.max(0,rescueState.reminderCooldown-dt);rescueState.allyCooldown=Math.max(0,rescueState.allyCooldown-dt);for(let i=0;i<RESCUE_PLAYERS.length;i++){if(rescueState.rescued.has(i))continue;const key=rescueKeys[i],cage=rescueCages[i],keyDistance=Math.hypot(hero.position.x-key.position.x,hero.position.z-key.position.z),cageDistance=Math.hypot(hero.position.x-cage.position.x,hero.position.z-cage.position.z);if(!rescueState.keys.has(i)&&keyDistance<1.45){rescueState.keys.add(i);key.visible=false;announce(`🔑 找到 ${RESCUE_PLAYERS[i].name} 的钥匙`);beep(720,.14);updateRescueHud()}if(!rescueState.keys.has(i)&&cageDistance<2&&rescueState.reminderCooldown<=0){announce(`先找到 ${RESCUE_PLAYERS[i].name} 的钥匙`);rescueState.reminderCooldown=1.4}if(rescueState.keys.has(i)&&cageDistance<2){rescueState.rescued.add(i);cage.visible=false;const follower=rescueFollowers[i];follower.position.copy(hero.position).add(new THREE.Vector3(0,0,-1));follower.visible=true;announce(`✓ 成功营救 ${RESCUE_PLAYERS[i].name} · 已加入队伍`);beep(940,.25);haptic([30,25,60]);updateRescueHud()}}
  const rescued=[...rescueState.rescued].sort((a,b)=>a-b),forwardX=Math.sin(hero.rotation.y),forwardZ=Math.cos(hero.rotation.y),rightX=forwardZ,rightZ=-forwardX;for(const [order,index] of rescued.entries()){const row=Math.floor(order/2)+1,lateral=(order%2?1:-1)*.95;rescueDesired.set(hero.position.x-forwardX*(1.1+row*1.05)+rightX*lateral,.02,hero.position.z-forwardZ*(1.1+row*1.05)+rightZ*lateral);const follower=rescueFollowers[index];follower.position.lerp(rescueDesired,1-Math.pow(.002,dt));follower.rotation.y=hero.rotation.y;const swing=Math.sin(t*.013+order)*.42;follower.userData.fLeg1.rotation.x=swing;follower.userData.fLeg2.rotation.x=-swing;follower.userData.fArm1.rotation.x=-swing;follower.userData.fArm2.rotation.x=swing}
  if(hunterState.active&&rescueState.allyCooldown<=0){const nearby=rescued.filter(index=>rescueFollowers[index].position.distanceTo(hunter.position)<5).length;if(nearby){const damage=nearby*5;hunterState.hp-=damage;hunterState.hitFlash=.16;rescueState.allyCooldown=.8;updateHunterHud();beep(420+nearby*18,.04);if(hunterState.hp<=0)defeatHunter()}}
}
function tick(t){requestAnimationFrame(tick);const dt=Math.min((t-state.last)/1000||0,.035);state.last=t;
  if(state.started&&!state.won&&!state.paused&&!toiletState.resolving){
    ...
    if(hunterState.active){
      ...
      if(hunterState.mode==='chase'&&chaseDistance>.1){const speed=hunterState.isMech?3.05:3.4+hunterState.stage*.1;moveHunter(chase.normalize(),speed,dt)}
      if(event==='strike'){
...
function returnToModes(){state.started=false;state.won=false;state.paused=false;resetToiletDoors(false);pauseMenu.classList.add('hidden');document.querySelector('#win').classList.add('hidden');mathPanel.classList.add('hidden');rescueHud.classList.add('hidden');hunter.visible=false;hunterHud.classList.add('hidden');attackButton.classList.add('hidden');for(const follower of rescueFollowers)follower.visible=false;audio.pause();document.querySelector('#start').classList.remove('hidden')}
...
skinReward.classList.toggle('hidden',skinProfile.claimedMedong);renderSkinGrid();applyQuality(qualityMode,false);showBest();reset(true);tick(0);
