...
import {MEDONG_PRAISE,MODE_CONFIG,RESCUE_BATTLE_OFFSETS,RESCUE_BUFF_EFFECTS,RESCUE_BUFF_LABELS,RESCUE_PLACEMENTS,RESCUE_PLAYERS,RESCUE_TRAIL_OFFSETS,SKIN_CATALOG,STAR_FACE_STYLE,STAR_FACE_TEXTURE_SIZE,TOILET_LAYOUT,createMathQuestion,isMathAnswerCorrect,isToiletNoCombat,requiredRescueForCheckpoint,rescueBuffTotals,rescueHunterHp,unlockedSkinIds} from './modes.js';
import {
  HERO_HEIGHT,
  HERO_RADIUS,
  intersectsBody,
...
function doAttack(){
  ...
  const buffs=state.mode==='rescue'?rescueBuffTotals(rescueState.rescued):rescueBuffTotals(),delta=attackDelta.copy(hunter.position).sub(hero.position),distance=Math.hypot(delta.x,delta.z),attackRange=3.45+buffs.strength*RESCUE_BUFF_EFFECTS.strengthRange;
...
function updateRescueHud(){const current=RESCUE_PLAYERS[state.stage],hasKey=rescueState.keys.has(state.stage),buffs=rescueBuffTotals(rescueState.rescued);rescueHud.querySelector('b').textContent=current&&!rescueState.rescued.has(state.stage)?hasKey?'🔑 已取得本区钥匙':`🔑 寻找 ${current.name} 的钥匙`:'🔑 当前区域已完成';rescueHud.querySelector('span').textContent=`已营救 ${rescueState.rescued.size} / ${RESCUE_PLAYERS.length}`;rescueBuffsEl.textContent=`攻击 +${buffs.attack} · 力量 +${buffs.strength} · 弹跳 +${buffs.jump} · 速度 +${buffs.speed}`}
...
function tick(t){requestAnimationFrame(tick);const dt=Math.min((t-state.last)/1000||0,.035);state.last=t;
  if(state.started&&!state.won&&!state.paused&&!toiletState.resolving){
    ...
    const dir=moveDirection.copy(viewForward).multiplyScalar(f).addScaledVector(viewRight,r),buffs=state.mode==='rescue'?rescueBuffTotals(rescueState.rescued):rescueBuffTotals(),moveSpeed=7.2+buffs.speed*RESCUE_BUFF_EFFECTS.speed;if(dir.length()>1)dir.normalize();state.velocity.x=dir.x*moveSpeed;state.velocity.z=dir.z*moveSpeed;
