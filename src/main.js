...
import {MEDONG_PRAISE,MODE_CONFIG,RESCUE_PLACEMENTS,RESCUE_PLAYERS,SKIN_CATALOG,STAR_FACE_STYLE,STAR_FACE_TEXTURE_SIZE,TOILET_LAYOUT,createMathQuestion,isMathAnswerCorrect,isToiletNoCombat,requiredRescueForCheckpoint,rescueHunterHp,unlockedSkinIds} from './modes.js';
import {
  HERO_HEIGHT,
  HERO_RADIUS,
  intersectsBody,
...
// 普通模式第 11～13 关：厕所入口、固定答案四选一隔间与下水道。
...
function createStarHead(player){const a=player.appearance,group=new THREE.Group(),skin=starMaterial(player,'skin'),hairMat=starMaterial(player,'hair'),accent=starMaterial(player,'accent'),crownMat=a.style==='bleached-tight-curls'?accent:hairMat,headMesh=cube(a.faceWidth,a.faceHeight,.7,[skin,skin,crownMat,skin,starFaceMaterial(player),skin]),top=a.faceHeight/2;group.add(headMesh);
  ...
  else{const fullBack=cube(a.faceWidth+.035,a.faceHeight+.02,.1,crownMat);fullBack.position.set(0,0,-.4);fullBack.name='modeled-full-short-hair-back';group.add(fullBack,createModeledSideHair(a.faceWidth,a.faceHeight,.7,hairMat))}
...
function applyHeroSkin(){const available=availableSkinIds();if(!available.has(skinProfile.selected))skinProfile.selected='prisoner';const skin=SKIN_CATALOG.find(item=>item.id===skinProfile.selected)||SKIN_CATALOG[0];for(const starHead of heroStarHeads)starHead.visible=false;heroMedongFrontHair.visible=heroMedongBackHair.visible=heroMedongSideHair.visible=false;head.visible=hair.visible=true;ponytail.visible=skin.id==='ponytail';arm1.material=arm2.material=M.skin;shoe1.material=shoe2.material=M.black;
  if(skin.id==='medong'){body.material=hBody.material;leg1.material=leg2.material=argentinaWhite;head.material=hHead.material;hair.material=ponytail.material=medongHair;heroMedongFrontHair.visible=heroMedongBackHair.visible=heroMedongSideHair.visible=true}
...
function reset(full=false){
  if(full||state.mode==='escape')resetToiletDoors();
  if(full){
