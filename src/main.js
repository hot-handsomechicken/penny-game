...
import {MEDONG_PRAISE,MODE_CONFIG,RESCUE_PLACEMENTS,RESCUE_PLAYERS,SKIN_CATALOG,createMathQuestion,isMathAnswerCorrect,requiredRescueForCheckpoint,rescueHunterHp,unlockedSkinIds} from './modes.js';
import {
  HERO_HEIGHT,
  HERO_RADIUS,
  intersectsBody,
...
const stageNames=['牢房区 · 01','洗衣封锁区 · 02','地下排污渠 · 03','锅炉房 · 04','安检走廊 · 05','监狱厨房 · 06','通风管道 · 07','放风操场 · 08','警戒屋顶 · 09','假撤离点 · 10','档案禁区 · 11','证物仓库 · 12','低温冷库 · 13','维修竖井 · 14','守卫营房 · 15','押运车库 · 16','外圈排水渠 · 17','探照灯广场 · 18','外墙脚手架 · 19','最终停机坪 · 20'];
const missions=['翻过牢房警戒线','避开横冲直撞的洗衣车','踩着浮箱越过污水渠','躲开旋转的高温管道','穿过交错的安检闸门','跳过灼热炉台','沿维修踏板爬出风管','穿过带刺封锁区','避开屋顶排风扇','突破假撤离点的封锁','穿过倒塌的档案架','越过封存证物箱','躲开滑动的冷冻货柜','沿维修平台穿过竖井','跳过营房警戒线','避开移动的押运车辆','踩着旧箱越过外圈污水','穿过交错的探照光束','沿脚手架登上外墙','击败梅东，抵达直升机'];
...
function reset(full=false){
  if(full){
    ...
    world.visible=state.mode!=='math';mathWorld.visible=state.mode==='math';rescueContent.visible=state.mode==='rescue';mathPanel.classList.toggle('hidden',state.mode!=='math');rescueHud.classList.toggle('hidden',state.mode!=='rescue');document.body.dataset.mode=state.mode;
...
function updateStage(i){if(state.mode==='math'||i<=state.stage)return;if(state.mode==='rescue'){const required=requiredRescueForCheckpoint(i);if(required!==null&&!rescueState.rescued.has(required)){hero.position.z=i*STAGE_LENGTH-.85;state.velocity.z=Math.min(0,state.velocity.z);if(rescueState.reminderCooldown<=0){announce(`你需要找到钥匙营救 ${RESCUE_PLAYERS[required].name} 球员！`);rescueState.reminderCooldown=1.4}return}}state.stage=i;state.checkpoint.set(0,.02,i*STAGE_LENGTH+2);stageEl.textContent=activeNames()[i];bar.style.width=`${(i+1)/activeStageCount()*100}%`;missionEl.textContent=activeMissions()[i];activeCheckpoints().forEach((c,k)=>setCheckpoint(c,k<=i));const fakeEscape=state.mode==='escape'&&i===10;announce(fakeEscape?'⚠ 假撤离点！后半程还有 10 关':i===activeLastStage()?`⚑ 最终检查点 · ${state.mode==='rescue'?'带领全员撤离':'准备撤离'}`:`⚑ 检查点 ${i+1} / ${activeStageCount()} 已激活`);beep(fakeEscape?330:740,.15);haptic(fakeEscape?[45,35,70]:45);if(state.mode==='rescue')updateRescueHud();if(encounterStages.includes(i)){if(hunterSpawnTimer)clearTimeout(hunterSpawnTimer);hunterSpawnTimer=setTimeout(()=>{hunterSpawnTimer=0;if(state.started&&!state.won&&state.stage===i)spawnHunter(i)},350)}}
...
function returnToModes(){state.started=false;state.won=false;state.paused=false;pauseMenu.classList.add('hidden');document.querySelector('#win').classList.add('hidden');mathPanel.classList.add('hidden');rescueHud.classList.add('hidden');hunter.visible=false;hunterHud.classList.add('hidden');attackButton.classList.add('hidden');for(const follower of rescueFollowers)follower.visible=false;audio.pause();document.querySelector('#start').classList.remove('hidden')}
